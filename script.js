...
const SPRITES = {
  girl: {
    element: girl,
    src: "assets/girl.png",
    crop: { x: 54, y: 82, width: 244, height: 514 }
  },
  boy: {
    element: boy,
    src: "assets/boy.png",
    crop: { x: 72, y: 78, width: 226, height: 520 }
  }
};

function isSheetBackground(data, index) {
  const red = data[index];
  const green = data[index + 1];
  const blue = data[index + 2];
  const max = Math.max(red, green, blue);
  const min = Math.min(red, green, blue);
  return red > 218 && green > 218 && blue > 218 && max - min < 22;
}

function floodTransparentFromEdges(imageData) {
  ...
  function enqueue(x, y) {
    ...
    if (!isSheetBackground(data, dataIndex)) return;
    visited[pixel] = 1;
    queue.push(pixel);
  }
...
async function prepareSprite({ element, src, crop }) {
...
window.addEventListener("load", () => {
  setTimeout(async () => {
    try {
      ...
      console.warn("Sprite cleanup failed; using original assets.", error);
    }
